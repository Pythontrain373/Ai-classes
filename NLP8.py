from Config6 import HF_API_KEY
...
def _exact_n_words(text: str, n: int):
...
def generste_text(prompt: str, max_new_tokens: int=220)-> str:
    raise Exception("Part 2 code not added")

def generate_exact_sentence(prompt: str, n_words: int, max_new_tokens:int,reies:int=6)->str:
    raise Exception("Part 2 code not added")
...
def main():
    ...
    if not os.path.exists(image_path):
        print(f"{Fore.RED}File '{image_path}' not found.")
        return
    try:
